...
# Licence-less gitleaks scan for NEAT-AI Explore (#609).
#
...
# scanning anything — the job goes green over an unscanned diff, which is worse
# than no gate at all because it reads as covered.
#
# `.github/workflows/gitleaks.yml` runs this script instead whenever the licence
# is absent. It fetches the free, open-source CLI at a pinned version, verifies
# it against its published SHA-256 checksum, and scans. Mirrors the canonical
# pattern in stSoftwareAU/NEAT-AI; each repository owns its own copy of the
# gate.
#
# Usage:
...
#   GITLEAKS_ASSET      Release asset name; defaults to the build matching this
#                       host.
...
# copy the matching lines out of that release's `*_checksums.txt`.
...
# ever executed. The scratch directory sits outside the workspace so the binary
# cannot be picked up by the repository's own file checks or staged by a later
# step. Echoes the path to the verified binary.
install_gitleaks() {
  local dir
  resolve_asset
  dir="$(mktemp -d)"
