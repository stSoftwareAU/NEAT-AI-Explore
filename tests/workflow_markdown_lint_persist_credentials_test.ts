/**
 * Tests that the markdown-lint workflow's checkout step does not persist the
 * GITHUB_TOKEN to disk (#461).
 *
 ...
 * token. The `markdownlint` job only reads Markdown files to lint them; it
 ...
 * `markdownlint` job sets `persist-credentials: false`.
 */
...
Deno.test(
  "markdownlint job checks out the repository (#461)",
  async () => {
    const wf = await loadWorkflow(MARKDOWN_LINT_WORKFLOW);
    const steps = checkoutSteps(wf.jobs?.markdownlint);
    assert(
      steps.length > 0,
      "markdownlint job must have an actions/checkout step",
    );
  },
);
...
Deno.test(
  ...
  async () => {
    const wf = await loadWorkflow(MARKDOWN_LINT_WORKFLOW);
    for (const step of checkoutSteps(wf.jobs?.markdownlint)) {
      assert(
        step.with !== undefined && step.with !== null,
        "markdownlint checkout must set `with.persist-credentials: false`",
      );
      assertEquals(
        step.with["persist-credentials"],
        false,
        "markdownlint checkout must set `persist-credentials: false` so the " +
