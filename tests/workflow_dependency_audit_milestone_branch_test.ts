/**
 * Tests that the Dependency Audit CI workflow runs on milestone PRs (#497).
 *
 ...
 * matches a `milestone/<slug>` branch — the Dependency Audit gate is silently
...
const WORKFLOW = "dependency-audit.yml";
...
function dependencyAuditBranches(wf: Workflow): string[] {
  const on = wf.on;
  assert(
    on !== undefined && typeof on === "object" && !Array.isArray(on),
    "dependency-audit.yml must declare an object-form `on:` block",
  );
  const pr = (on as WorkflowTriggers).pull_request;
  assert(
    pr !== undefined && pr !== null,
    "dependency-audit.yml must keep its pull_request trigger so it gates PRs",
  );
  return pr.branches ?? [];
}

Deno.test(
  "dependency-audit.yml runs on milestone/<slug> PRs (#497)",
  async () => {
    const wf = await loadWorkflow(WORKFLOW);
    const branches = dependencyAuditBranches(wf);
    for (const branch of ["milestone/foo", "milestone/issue-497-fix"]) {
      assert(
        anyBranchMatches(branches, branch),
        `dependency-audit.yml pull_request.branches must match '${branch}' so the ` +
...
Deno.test(
  "dependency-audit.yml still runs on ordinary single-level branches (#497)",
  async () => {
    const wf = await loadWorkflow(WORKFLOW);
    const branches = dependencyAuditBranches(wf);
    for (const branch of ["Develop", "main", "feature-x"]) {
      assert(
        anyBranchMatches(branches, branch),
        `dependency-audit.yml pull_request.branches must still match '${branch}'. ` +
