/**
 * Tests that the actionlint CI quality workflow runs on milestone PRs (#492).
 *
 ...
 * branch is checked.
 *
 * These tests assert that the actionlint workflow's `pull_request.branches`
...
function actionlintBranches(wf: Workflow): string[] {
  const on = wf.on;
  assert(
...
Deno.test(
  "actionlint.yml runs on milestone/<slug> PRs (#492)",
  async () => {
    const wf = await loadWorkflow(ACTIONLINT_WORKFLOW);
    const branches = actionlintBranches(wf);
    for (const branch of ["milestone/foo", "milestone/issue-492-fix"]) {
      assert(
        anyBranchMatches(branches, branch),
        `actionlint.yml pull_request.branches must match '${branch}' so the ` +
...
Deno.test(
  ...
  async () => {
    ...
    for (const branch of ["Develop", "main", "feature-x"]) {
      assert(
        anyBranchMatches(branches, branch),
        `actionlint.yml pull_request.branches must still match '${branch}'. ` +
