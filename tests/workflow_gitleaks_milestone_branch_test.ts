/**
 * Tests that the Gitleaks CI workflow runs on milestone PRs (#494).
 *
 ...
 * matches a `milestone/<slug>` branch — the secret-scan gate is silently
 * skipped on every intermediate sub-issue PR and only the single rollup PR into
 * the default branch is checked.
 *
 * These tests assert that the gitleaks workflow's `pull_request.branches`
...
function gitleaksBranches(wf: Workflow): string[] {
  const on = wf.on;
  assert(
    on !== undefined && typeof on === "object" && !Array.isArray(on),
    "gitleaks.yml must declare an object-form `on:` block",
  );
  const pr = (on as WorkflowTriggers).pull_request;
  assert(
    pr !== undefined && pr !== null,
    "gitleaks.yml must keep its pull_request trigger so it gates PRs",
  );
  return pr.branches ?? [];
}

Deno.test(
  "gitleaks.yml runs on milestone/<slug> PRs (#494)",
  async () => {
    const wf = await loadWorkflow(GITLEAKS_WORKFLOW);
    const branches = gitleaksBranches(wf);
    for (const branch of ["milestone/foo", "milestone/issue-494-fix"]) {
      assert(
        anyBranchMatches(branches, branch),
        `gitleaks.yml pull_request.branches must match '${branch}' so the ` +
...
Deno.test(
  ...
  async () => {
    ...
    for (const branch of ["Develop", "main", "feature-x"]) {
      assert(
        anyBranchMatches(branches, branch),
        `gitleaks.yml pull_request.branches must still match '${branch}'. ` +
