/**
 * Tests that the Markdown Lint CI workflow runs on milestone PRs (#495).
 *
 ...
 * branch is checked.
 *
 * These tests assert that the markdown-lint workflow's `pull_request.branches`
 * filter matches milestone branches while still matching ordinary
 * single-level branches (e.g. `Develop`), so the fix does not regress the
 * existing coverage.
 */
...
const MARKDOWN_LINT_WORKFLOW = "markdown-lint.yml";
...
function markdownLintBranches(wf: Workflow): string[] {
  const on = wf.on;
  assert(
    on !== undefined && typeof on === "object" && !Array.isArray(on),
    "markdown-lint.yml must declare an object-form `on:` block",
  );
  const pr = (on as WorkflowTriggers).pull_request;
  assert(
    pr !== undefined && pr !== null,
    "markdown-lint.yml must keep its pull_request trigger so it gates PRs",
  );
  return pr.branches ?? [];
}

Deno.test(
  "markdown-lint.yml runs on milestone/<slug> PRs (#495)",
  async () => {
    const wf = await loadWorkflow(MARKDOWN_LINT_WORKFLOW);
    const branches = markdownLintBranches(wf);
    for (const branch of ["milestone/foo", "milestone/issue-495-fix"]) {
      assert(
        anyBranchMatches(branches, branch),
        `markdown-lint.yml pull_request.branches must match '${branch}' so the ` +
...
Deno.test(
  "markdown-lint.yml still runs on ordinary single-level branches (#495)",
  async () => {
    const wf = await loadWorkflow(MARKDOWN_LINT_WORKFLOW);
    const branches = markdownLintBranches(wf);
    for (const branch of ["Develop", "main", "feature-x"]) {
      assert(
        anyBranchMatches(branches, branch),
        `markdown-lint.yml pull_request.branches must still match '${branch}'. ` +
