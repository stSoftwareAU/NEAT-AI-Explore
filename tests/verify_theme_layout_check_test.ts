 ...
 *
 * Mirrors `tests/verify_starfield_layout_check_test.ts`: runs
 * `deno check scripts/verify_theme_layout.ts` as a subprocess and asserts a
 * zero exit code so any future regression in the Deno port (e.g. an
 * unresolvable playwright version, a stale npm specifier, or a type
...
Deno.test("scripts/verify_theme_layout.ts uses bare 'playwright' specifier", async () => {
  const src = await Deno.readTextFile(
    new URL("../scripts/verify_theme_layout.ts", import.meta.url),
  );
  ...
  assert(
    ...
    "verify_theme_layout.ts must import 'playwright' via the deno.json imports map, not via an inline 'npm:' specifier",
  );
  assert(
    /from\s+["']playwright["']/.test(src),
    "verify_theme_layout.ts must import the bare 'playwright' specifier",
  );
});
