/**
 * Tests that the ShellCheck CI workflow runs on milestone PRs (#497).
 *
 ...
 * matches a `milestone/<slug>` branch — the ShellCheck gate is silently
...
function shellcheckBranches(wf: Workflow): string[] {
  const on = wf.on;
  assert(
    on !== undefined && typeof on === "object" && !Array.isArray(on),
    "shellcheck.yml must declare an object-form `on:` block",
  );
  const pr = (on as WorkflowTriggers).pull_request;
  assert(
    pr !== undefined && pr !== null,
    "shellcheck.yml must keep its pull_request trigger so it gates PRs",
  );
  return pr.branches ?? [];
}

Deno.test(
  "shellcheck.yml runs on milestone/<slug> PRs (#497)",
  async () => {
    const wf = await loadWorkflow(WORKFLOW);
    const branches = shellcheckBranches(wf);
    for (const branch of ["milestone/foo", "milestone/issue-497-fix"]) {
      assert(
        anyBranchMatches(branches, branch),
        `shellcheck.yml pull_request.branches must match '${branch}' so the ` +
...
Deno.test(
  "shellcheck.yml still runs on ordinary single-level branches (#497)",
  async () => {
    const wf = await loadWorkflow(WORKFLOW);
    const branches = shellcheckBranches(wf);
    for (const branch of ["Develop", "main", "feature-x"]) {
      assert(
        anyBranchMatches(branches, branch),
        `shellcheck.yml pull_request.branches must still match '${branch}'. ` +
