/**
 * Tests that the Bash Syntax CI workflow runs on milestone PRs (#497).
 *
 ...
 * matches a `milestone/<slug>` branch — the Bash Syntax gate is silently
...
const WORKFLOW = "bash-syntax.yml";
...
function bashSyntaxBranches(wf: Workflow): string[] {
  const on = wf.on;
  assert(
    on !== undefined && typeof on === "object" && !Array.isArray(on),
    "bash-syntax.yml must declare an object-form `on:` block",
  );
  const pr = (on as WorkflowTriggers).pull_request;
  assert(
    pr !== undefined && pr !== null,
    "bash-syntax.yml must keep its pull_request trigger so it gates PRs",
  );
  return pr.branches ?? [];
}

Deno.test(
  "bash-syntax.yml runs on milestone/<slug> PRs (#497)",
  async () => {
    const wf = await loadWorkflow(WORKFLOW);
    const branches = bashSyntaxBranches(wf);
    for (const branch of ["milestone/foo", "milestone/issue-497-fix"]) {
      assert(
        anyBranchMatches(branches, branch),
        `bash-syntax.yml pull_request.branches must match '${branch}' so the ` +
...
Deno.test(
  "bash-syntax.yml still runs on ordinary single-level branches (#497)",
  async () => {
    const wf = await loadWorkflow(WORKFLOW);
    const branches = bashSyntaxBranches(wf);
    for (const branch of ["Develop", "main", "feature-x"]) {
      assert(
        anyBranchMatches(branches, branch),
        `bash-syntax.yml pull_request.branches must still match '${branch}'. ` +
