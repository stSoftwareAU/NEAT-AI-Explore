/**
 * Tests that the markdown-lint checker workflow does not re-run on push to the
 * default branch (#454).
 *
 * `markdown-lint.yml` is a test/lint workflow: it gates pull requests. When it
 ...
 *
 * These tests assert that markdown-lint.yml keeps its `pull_request` trigger
 * and no longer pushes on the default branch.
 */
...
Deno.test(
  "markdown-lint.yml still gates pull requests (#454)",
  async () => {
...
Deno.test(
  ...
  async () => {
    ...
    assert(
      ...
      `markdown-lint.yml must not trigger on push to the default branch ` +
