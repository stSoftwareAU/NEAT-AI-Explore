 ...
 * token. The `quality` job only reads the repo to run format/lint/type/test
 * checks; it never pushes back to the repo or fetches private submodules, so
 * the token does not need to persist. Setting `persist-credentials: false`
