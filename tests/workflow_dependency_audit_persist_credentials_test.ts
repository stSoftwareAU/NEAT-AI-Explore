/**
 * Tests that the dependency-audit workflow's checkout step does not persist the
 * GITHUB_TOKEN to disk (#458).
 *
 ...
 * token. The `audit` job only checks out the lockfile and runs `deno audit`; it
 ...
 * `audit` job sets `persist-credentials: false`.
 */
...
Deno.test(
  "audit job checks out the repository (#458)",
  async () => {
    const wf = await loadWorkflow(AUDIT_WORKFLOW);
    const steps = checkoutSteps(wf.jobs?.audit);
    assert(
      steps.length > 0,
      "audit job must have an actions/checkout step",
    );
  },
);
...
Deno.test(
  ...
  async () => {
    const wf = await loadWorkflow(AUDIT_WORKFLOW);
    for (const step of checkoutSteps(wf.jobs?.audit)) {
      assert(
        step.with !== undefined && step.with !== null,
        "audit checkout must set `with.persist-credentials: false`",
      );
      assertEquals(
        step.with["persist-credentials"],
        false,
        "audit checkout must set `persist-credentials: false` so the " +
