/**
 * Tests that the Deno quality CI workflow runs on milestone PRs (#493).
 *
 ...
 * branch is checked.
 *
 * These tests assert that the deno-quality workflow's `pull_request.branches`
...
function denoQualityBranches(wf: Workflow): string[] {
  const on = wf.on;
  assert(
    on !== undefined && typeof on === "object" && !Array.isArray(on),
    "deno-quality.yml must declare an object-form `on:` block",
  );
  const pr = (on as WorkflowTriggers).pull_request;
  assert(
    pr !== undefined && pr !== null,
    "deno-quality.yml must keep its pull_request trigger so it gates PRs",
  );
  return pr.branches ?? [];
}

Deno.test(
  "deno-quality.yml runs on milestone/<slug> PRs (#493)",
  async () => {
    const wf = await loadWorkflow(DENO_QUALITY_WORKFLOW);
    const branches = denoQualityBranches(wf);
    for (const branch of ["milestone/foo", "milestone/issue-493-fix"]) {
      assert(
        anyBranchMatches(branches, branch),
        `deno-quality.yml pull_request.branches must match '${branch}' so the ` +
...
Deno.test(
  ...
  async () => {
    ...
    for (const branch of ["Develop", "main", "feature-x"]) {
      assert(
        anyBranchMatches(branches, branch),
        `deno-quality.yml pull_request.branches must still match '${branch}'. ` +
