/**
 * Tests that the Semgrep CI workflow runs on milestone PRs (#496).
 *
 ...
 * matches a `milestone/<slug>` branch — the Semgrep scan gate is silently
 * skipped on every intermediate sub-issue PR and only the single rollup PR into
 * the default branch is checked.
 *
 * These tests assert that the semgrep workflow's `pull_request.branches`
 * filter matches milestone branches while still matching ordinary single-level
 * branches (e.g. `Develop`), so the fix does not regress the existing coverage.
 */
...
const SEMGREP_WORKFLOW = "semgrep.yml";
...
function semgrepBranches(wf: Workflow): string[] {
  const on = wf.on;
  assert(
    on !== undefined && typeof on === "object" && !Array.isArray(on),
    "semgrep.yml must declare an object-form `on:` block",
  );
  const pr = (on as WorkflowTriggers).pull_request;
  assert(
    pr !== undefined && pr !== null,
    "semgrep.yml must keep its pull_request trigger so it gates PRs",
  );
  return pr.branches ?? [];
}

Deno.test(
  "semgrep.yml runs on milestone/<slug> PRs (#496)",
  async () => {
    const wf = await loadWorkflow(SEMGREP_WORKFLOW);
    const branches = semgrepBranches(wf);
    for (const branch of ["milestone/foo", "milestone/issue-496-fix"]) {
      assert(
        anyBranchMatches(branches, branch),
        `semgrep.yml pull_request.branches must match '${branch}' so the ` +
...
Deno.test(
  "semgrep.yml still runs on ordinary single-level branches (#496)",
  async () => {
    const wf = await loadWorkflow(SEMGREP_WORKFLOW);
    const branches = semgrepBranches(wf);
    for (const branch of ["Develop", "main", "feature-x"]) {
      assert(
        anyBranchMatches(branches, branch),
        `semgrep.yml pull_request.branches must still match '${branch}'. ` +
