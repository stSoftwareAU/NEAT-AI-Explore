/**
 * Tests that the semgrep workflow's checkout step does not persist the
 * GITHUB_TOKEN to disk (#462).
 *
 ...
 * token. The `semgrep` job only checks out the repo to scan it; it never
 ...
 * `semgrep` job sets `persist-credentials: false`.
 */
...
Deno.test(
  "semgrep job checks out the repository (#462)",
  async () => {
    const wf = await loadWorkflow(SEMGREP_WORKFLOW);
    const steps = checkoutSteps(wf.jobs?.semgrep);
    assert(
      steps.length > 0,
      "semgrep job must have an actions/checkout step",
    );
  },
);

Deno.test(
  "semgrep checkout does not persist the GITHUB_TOKEN to disk (#462)",
  async () => {
    const wf = await loadWorkflow(SEMGREP_WORKFLOW);
    for (const step of checkoutSteps(wf.jobs?.semgrep)) {
      assert(
        step.with !== undefined && step.with !== null,
        "semgrep checkout must set `with.persist-credentials: false`",
      );
      assertEquals(
        step.with["persist-credentials"],
        false,
        "semgrep checkout must set `persist-credentials: false` so the " +
