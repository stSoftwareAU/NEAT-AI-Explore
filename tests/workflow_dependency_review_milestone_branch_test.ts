/**
 * Tests that the Dependency Review CI workflow runs on milestone PRs (#497).
 *
 ...
 * matches a `milestone/<slug>` branch — the Dependency Review gate is silently
...
function dependencyReviewBranches(wf: Workflow): string[] {
  const on = wf.on;
  assert(
    on !== undefined && typeof on === "object" && !Array.isArray(on),
    "dependency-review.yml must declare an object-form `on:` block",
  );
  const pr = (on as WorkflowTriggers).pull_request;
  assert(
    pr !== undefined && pr !== null,
    "dependency-review.yml must keep its pull_request trigger so it gates PRs",
  );
  return pr.branches ?? [];
}

Deno.test(
  "dependency-review.yml runs on milestone/<slug> PRs (#497)",
  async () => {
    const wf = await loadWorkflow(WORKFLOW);
    const branches = dependencyReviewBranches(wf);
    for (const branch of ["milestone/foo", "milestone/issue-497-fix"]) {
      assert(
        anyBranchMatches(branches, branch),
        `dependency-review.yml pull_request.branches must match '${branch}' so the ` +
...
Deno.test(
  "dependency-review.yml still runs on ordinary single-level branches (#497)",
  async () => {
    const wf = await loadWorkflow(WORKFLOW);
    const branches = dependencyReviewBranches(wf);
    for (const branch of ["Develop", "main", "feature-x"]) {
      assert(
        anyBranchMatches(branches, branch),
        `dependency-review.yml pull_request.branches must still match '${branch}'. ` +
