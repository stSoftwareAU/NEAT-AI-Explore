...
deprecated Node.js 20 runtime. Node 20 is scheduled for removal on GitHub-hosted
runners on 2026-09-16, so the runner emitted a Node 20 deprecation warning.

This change bumps the pin to `45bfe0192ca1faeb007ade9deae92b16b8254a0d`
(v6.0.0), the current major, which runs on the supported **Node 24** runtime.
The reference stays pinned to a 40-char commit SHA per the SHA-pinning policy
(#190).
...
- Confirmed v6.0.0 is the current major and runs on Node 24:
  `actions/configure-pages` `action.yml` at `v6.0.0` declares `using: 'node24'`;
  the tag resolves to commit `45bfe0192ca1faeb007ade9deae92b16b8254a0d`.
