...
(`#legend`). The legend already carried `aria-label="Graph legend"`, but the
HUD had no accessible name — `aria-live="polite"` announces content changes
but does not name the landmark. A screen-reader user navigating by landmark
therefore saw two undistinguished "complementary" regions per page and could
not tell the Focus HUD from the Legend.
...
attribute and does not alter rendered layout or styling, so there is no
visual diff to screenshot. Correctness is verified by the DOM-based tests
below, which parse the published HTML and assert the landmark contract.
...
- Added `tests/hud_aside_label_test.ts`, which for both `docs/graph/index.html`
  ...
  - asserts the `<aside id="hud">` landmark carries `aria-label="Focus details"`;
...
- Confirmed the tests fail against the unfixed markup and pass after adding
  the `aria-label`.
