...
Changes:
- `import { chromium } from "playwright";` → `import { chromium, type Page } from "playwright";`
...
- `deno check scripts/generate_pwa_assets.ts` → `Check scripts/generate_pwa_assets.ts` (no errors).
- `./quality.sh` passes: fmt, lint, whole-repo type check, and 740 tests all green.
...
- Existing `tests/generate_pwa_assets_check_test.ts::deno check scripts/generate_pwa_assets.ts exits cleanly`
  ...
  errors. It passes against the real Playwright `Page` type, confirming the
  cast removal did not introduce a type mismatch. No source-grep tests were
  added (they would break on any refactor and are disallowed by the repo's
  testing guidelines).
