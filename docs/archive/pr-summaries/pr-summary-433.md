...
### Caveat review — is the flag _meant_ to be cleared on success?
...
- The recovery flag lives in **`sessionStorage`** (key
  ...
  `docs/boot.js`.
- The module's own documentation states the flag exists so we _"only attempt
  once per session"_ and to _"prevent an infinite recovery loop"_.
- `sessionStorage` is cleared automatically by the browser when the session ends
  (tab/app close), so the once-per-session lifecycle needs **no explicit
...
`./quality.sh` passes cleanly: `757 passed | 0 failed`, format/lint/type checks
OK.

## Test Plan

- Removed the now-obsolete
  `tests/pwa_recovery_test.ts::"clearRecoveryFlag
  removes the flag"` test and
  the `clearRecoveryFlag` import (business-logic change: the export it exercised
  no longer exists).
