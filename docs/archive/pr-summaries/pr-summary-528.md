...
- **New comparison hub `/compare/`** — loads one snapshot URL and wires every
  ...
  Discoverable from the overview dashboard's _Compare graph views_ action.
- **New top-impact subgraph view `/subgraph/`** — the third candidate (#527) did
  not yet exist, and the comparison's "all three reachable" acceptance needs it.
  It keeps only the highest-impact nodes on the paths to the Score and reports
  everything it drops as a candidate **dead zone** (2,154 of 2,167 aggregated
  nodes on the default snapshot), with a `Top paths (N)` slider.
...
- **Documented evaluation + winner** —
  `docs/archive/candidate-view-evaluation-528.md` scores each candidate against
  the three #522 goals plus phone-friendliness and future per-stock growth, and
  recommends the **layered DAG** as the primary replacement (keeping the Sankey
  and subgraph as complementary lenses).
...
- `tests/candidate_comparison_test.ts` — the chooser lists all three candidates;
  each link propagates the snapshot and round-trips through
