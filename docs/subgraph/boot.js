// Boot script for the top-impact subgraph view (docs/subgraph/index.html) —
// Issue #527.
//
// Mirrors docs/dag/boot.js: the Service Worker is registered *before* the view
// module is imported so the first snapshot fetch does not race SW activation,
// and a failed module import self-heals the PWA caches (Issue #194) instead of
...
(async () => {
  ...
  try {
    await import(`./subgraph.js?v=${v}`);
  } catch (err) {
    console.error("Subgraph module failed to load:", err);
